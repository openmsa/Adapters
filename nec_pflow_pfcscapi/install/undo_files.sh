...
if [ $? -eq 0 ]; then
  ...
  grep -v "^17111601," /opt/ubi-jentreprise/resources/templates/conf/device/models.properties > /tmp/msa.$$
...
if [ $? -eq 0 ]; then
  ...
  grep -v "necPfcScApi" /opt/ses/properties/specifics/server_ALL/sdExtendedInfo.properties > /tmp/msa.$$
...
if [ $? -eq 0 ]; then
  ...
  sed 's/ necPfcScApi//' /opt/ses/properties/specifics/server_ALL/manageLinks.properties > /tmp/msa.$$
