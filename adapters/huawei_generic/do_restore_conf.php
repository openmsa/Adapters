...
require_once load_once('huawei_generic', 'device_restore_configuration.php');
...
try
{
	...
	$conf = new device_restore_configuration($sdid);
	...
	sleep(15);
	$ret = $conf->remove_old_config();
	if ($ret !== SMS_OK)
	{
		sms_set_update_status($sms_csp, $sdid, $ret, 'RESTORE', 'FAILED', "Delete of old configuration file failed (restore revision: $revision_id)");
		sms_sd_unlock($sms_csp, $sms_sd_info);
		return SMS_OK;
	}
	
	require_once load_once('huawei_generic', 'do_backup_conf.php');
