...
require_once load_once ( 'netconf_generic', 'netconf_generic_apply_conf.php' );
require_once load_once ( 'netconf_generic', 'netconf_generic_apply_restore_conf.php' );

require_once "$db_objects";
class netconf_generic_configuration {
	...
	function get_running_conf() {
		...
		$rpc_get_running = PATTERNIZETEMPLATE("get_running_rpc.tpl");

		$config_all = sendexpectone(__FILE__.':'.__LINE__, $sms_sd_ctx, $rpc_get_running);
		...
		} else // use the repository
{
	...
	function restore_conf($configuration) {
		$ret = netconf_generic_apply_restore_conf ( $configuration );
		return $ret;
	}
	...
	function build_conf(&$generated_configuration) {
		...
		$generated_configuration = $this->build_rpc_conf($generated_configuration);

		return SMS_OK;
	}

	function build_rpc_conf(&$generated_configuration) {

		$config_to_push = PATTERNIZETEMPLATE("rpc_start.tpl");
		$config_to_push .= $generated_configuration;
		$config_to_push .= PATTERNIZETEMPLATE("rpc_end.tpl");
	...
	function update_conf() {
		...
		if (! empty ( $generated_configuration )) {
			$ret = netconf_generic_apply_conf ( $generated_configuration );
		}
