...
function get_citrix_netscaler_error($err_string)
{
    $error_citrix_netscaler_string = htmlentities(substr($err_string, strpos($err_string, 'ERROR:'), (strlen($err_string) - strpos($err_string, '*****'))*(-1)));
    return $error_citrix_netscaler_string;
}
...
function func_reboot()
{
  ...
  {
    $cmd_line = "Y";
    $index = sendexpect(__FILE__ . ':' . __LINE__, $sms_sd_ctx, $cmd_line, $tab[1]);
  }
}
...
function func_write()
{
  ...
  /**
   * TODO : Include the scenario when save config fails. In $tab[1]
   */
  unset($tab);
  $tab[0] = 'Done';
  $tab[1] = '';
  $index = sendexpect(__FILE__ . ':' . __LINE__, $sms_sd_ctx, "save ns config", $tab);
  if ($index !== 0)
  {
