...
class mysql_generic_command extends generic_command
{
  var $parser_list;
  var $parsed_objects;
  var $create_list;
      ...
      if (!empty($this->parser_list))
      {
        ...
        foreach ($this->parser_list as $operation => $parsers)
        {
          ...
          foreach ($sub_list as $op_eval => $sub_parsers)
          {
            ...
            {
	      $op = "mysql --host={$SD->SD_IP_CONFIG} --user={$SD->SD_LOGIN_ENTRY} --password='{$SD->SD_PASSWD_ENTRY}' --database={$SD->SD_CONFIGVAR_list['DATABASE']->VAR_VALUE} --batch --execute='{$op}' | tail -n +2";
