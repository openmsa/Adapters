...
class DeviceConnection extends GenericConnection {
	...
	public function send($origin, $rest_cmd) {
		...
		if(!isset($this->key)){
			...
			$curl_cmd = "curl " . $auth . " {$headers} --connect-timeout {$this->conn_timeout} --max-time {$this->conn_timeout} -X {$http_op} -k '{$this->protocol}://eu01.sb.infonova.com/auth/realms/SB_NTT_BUSINESSPLACE/protocol/openid-connect{$rest_path}'";
                	if (count($cmd_list) >2 ) {
			...
			$curl_cmd = "curl " . $auth . "{$headers} -H 'Content-Type: application/json' --connect-timeout {$this->conn_timeout} --max-time {$this->conn_timeout} -X {$http_op} -k '{$this->protocol}://eu01.sb.infonova.com/r6-api{$rest_path}'";
	                if (count($cmd_list) >2 ) {
        	                $rest_payload = $cmd_list[2];
                	        $curl_cmd .= " -d ";
                        	$curl_cmd .= "'{$rest_payload}'";
	...
	protected function execute_curl_command($origin, $rest_cmd, $curl_cmd) {
		...
		if (strpos($curl_cmd, "Content-Type: application/json")) {
			...
			while(preg_match('/"([^,:]+)\s+(\S+)":/',$result)){
				$result=preg_replace('/"([^,:]+)\s+(\S+)":/', '"$1--$2":', $result);
			}
...
class TokenConnection extends DeviceConnection {
	...
	public function do_connect() {
		...
		if($this->auth_mode != "auth-key")
		{
			...
			//$data = 'client_id=devclient_apolz&client_secret=Seering!6&grant_type=password&username=anhyper_api&password=anhyper!#
                        $data = 'client_id=devclient_apolz&client_secret=Seering!6&grant_type=password&username='."$this->sd_login_entry".'&password='."{$this->sd_passwd_entry}";
