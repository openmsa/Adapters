...
require_once load_once ( 'adtran_generic', 'adtran_generic_connect_port.php' );
...
function sd_connect($login = null, $passwd = null, $adminpasswd = null, $ts_ip = null, $ts_port = null) {
	if (empty ( $ts_ip ) || empty ( $ts_port )) {
		...
		$ret = adtran_generic_connect_port ( $ts_ip, $ts_port, $adminpasswd );
	}
...
function sd_disconnect($clean_exit = false, $ts_ip = null) {
	if (empty ( $ts_ip )) {
		$ret = adtran_generic_disconnect ( $clean_exit );
	} else {
		$ret = adtran_generic_disconnect_port ( $clean_exit );
	}
...
function sd_apply_conf($configuration, $need_sd_connection = false, $push_to_startup = false, $ts_ip = null, $ts_port = null) {
	...
	$ret = adtran_generic_apply_conf ( $configuration, $push_to_startup );
...
function sd_save_conf() {
	...
	$conf = new adtran_generic_configuration ( $sdid );
