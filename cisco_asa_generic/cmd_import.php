...
class cmd_import
{
  ...
  function parse($buffer, &$objects)
  {
    ...
    while ($line !== false)
    {
      ...
      if (!$is_in_section)
      {
        ...
        {
          $this->parse_lines_object_id($line, $this->parser['lines'], $config, $objects);
        }
      }
            ...
            else if ($rule === 'regexp')
            {
              ...
              foreach ($rule_content as $regexp)
              {
                if (regexp_match("Object template {$this->object_name}", $regexp, $line, $matches) > 0)
                {
                  ...
                  debug_dump($sub_object, "SUB_OBJECT");
                  break;
                }
              }
            }
  ...
  function parse_lines_object_id(&$theline, $parser, &$config, &$objects)
  {
    $var_pattern = '/\(\?\<(\w+)\>/';
    $found = false;
    $line = trim($theline);
          ...
          if ($found)
          {
            ...
            else
            {
              ...
              if (!$is_multiple)
              {
                ...
                {
                  $index = $sub_object['index'];
                  if (!empty($objarray[$index]) && is_array($objarray[$index]))
                  {
                    $arr = array_merge($objarray[$index], $sub_object);
                    $objarray[$index] = $arr;
                  }
