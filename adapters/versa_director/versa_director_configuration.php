...
//require_once load_once('versa_director', 'common.php');
require_once load_once('versa_director', 'versa_director_apply_conf.php');
//require_once load_once('versa_director', 'versa_director_apply_restore_conf.php');


require_once "$db_objects";
class versa_director_configuration
{
  ...
  function get_running_conf()
  {
    ...
    $result = sendexpectone(__FILE__ . ':' . __LINE__, $sms_sd_ctx, $xpath, $result);
    $SMS_OUTPUT_BUF = $sms_sd_ctx->get_raw_xml();
    $config_string = $result->asXml();

    $this->running_conf = $SMS_OUTPUT_BUF;
  ...
  {
    $ret = versa_director_apply_restore_conf($configuration);
    return $ret;
  }
  ...
  function update_conf()
  {
    ...
    {
      $ret = versa_director_apply_conf($generated_configuration);
    }
