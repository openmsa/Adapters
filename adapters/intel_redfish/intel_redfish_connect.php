    ...
    protected function execute_curl_command($origin, $rest_cmd, $curl_cmd) {
        ...
        while ($is_it_success === False and $attempt_countdown > 0) {
            ...
            if ($is_it_success === False) {
                debug_dump($attempt_countdown, "DEBUG: We've got wrong reply from server. Sleeping 5 sec. Attempt ");
		--$attempt_countdown;
		sleep(5);
            }
        }
...
function intel_redfish_connect($sd_ip_addr = null, $login = null, $passwd = null, $port_to_use = null) {
    global $sms_sd_ctx;
    global $model_data;
...
// Disconnect
function intel_redfish_disconnect() {
    global $sms_sd_ctx;
    $sms_sd_ctx = null;
    return SMS_OK;
}
