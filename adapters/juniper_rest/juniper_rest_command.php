...
require_once load_once ( 'juniper_rest', 'adaptor.php' );
class juniper_rest_command extends generic_command {
	var $parser_list;
	var $parsed_objects;
	var $create_list;
	var $delete_list;
		...
		try {
			...
			if (! empty ( $this->parser_list )) {
				...
				foreach ( $parser_list as $op_eval => $sub_parsers ) {
					...
					//remove namespaces and another attributes from XML tag
					debug_dump($running_conf, "DEBUG: RUNNING CONF");
