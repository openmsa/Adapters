...
require_once load_once('kubernetes_generic', 'kubernetes_generic_apply_conf.php');
...
{
  $ret = kubernetes_generic_connect($login, $passwd);
  return $ret;
}
...
{
  $ret = kubernetes_generic_disconnect();
  return $ret;
}
...
function sd_apply_conf($configuration, $need_sd_connection)
{
  ...
  $ret = kubernetes_generic_apply_conf($configuration);
