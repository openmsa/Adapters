...
function redfish_generic_connect($sd_ip_addr = null, $login = null, $passwd = null, $port_to_use = null) {
	...
	echo "redfish_generic_connect: using connection class: " . $class . "\n";
	$sms_sd_ctx = new $class ( $sd_ip_addr, $login, $passwd, $port_to_use );
	echo  "redfish_generic_connect: setting authentication mode to: {$auth_mode}\n";
	...
	if ($sms_sd_ctx->auth_mode == "token") {
		...
		echo  "redfish_generic_connect: setting SIGNIN_REQ_PATH to: {$sms_sd_ctx->sign_in_req_path}\n";

		if (isset($sd->SD_CONFIGVAR_list['TOKEN_XPATH'])) {
			$sms_sd_ctx->token_xpath = $sd->SD_CONFIGVAR_list['TOKEN_XPATH']->VAR_VALUE;
		}
		echo  "redfish_generic_connect: setting TOKEN_XPATH to: {$sms_sd_ctx->token_xpath}\n";
		...
		echo  "redfish_generic_connect: setting authentication header to: {$sms_sd_ctx->auth_header}\n";
	} 
