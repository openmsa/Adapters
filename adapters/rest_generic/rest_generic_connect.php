...
class TokenConnection extends DeviceConnection {
	...
	public function do_connect() {
		...
		if($this->auth_mode != "auth-key")
		{
			...
			$result = $this->sendexpectone ( _FILE_ . ':' . _LINE_, $cmd );
...
function rest_generic_connect($sd_ip_addr = null, $login = null, $passwd = null, $port_to_use = null) {
	...
	if ($sms_sd_ctx->auth_mode == "token" || $sms_sd_ctx->auth_mode == "auth-key" ) {
		...
		if (!isset($sd->SD_CONFIGVAR_list['SIGNIN_REQ_PATH'])) {
			throw new SmsException ( _FILE_ . ':' . _LINE_." missing value for config var SIGNIN_REQ_PATH" , ERR_SD_CMDFAILED);
		}
		...
		if (!isset($sd->SD_CONFIGVAR_list['AUTH_HEADER'])) {
			throw new SmsException ( _FILE_ . ':' . _LINE_." missing value for config var AUTH_HEADER" , ERR_SD_CMDFAILED);
		}
