...
require_once load_once('cisco_ftd_rest', 'me_restore_configuration.php');
...
try
{
  $restore = new MeRestoreConfiguration($sdid);
  ...
  $restore->get_revision($revision_id);
  ...
  sms_set_update_status($sms_csp, $sdid, SMS_OK, $status_type, 'WORKING', "Finalizing restore (restore revision: $revision_id)");

  $restore->finalize_restore();
