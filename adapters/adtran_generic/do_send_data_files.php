...
try {
  if (!empty($map_repo))
  {
    ...
    {
    	if(preg_match($disk_name, implode($map_repo)) || substr_count(implode($map_repo), 'extract') > 0)
    	{
    ...
    if ($flash_file_transfer)
    {
      ...
      foreach ($map_repo as $file)
      {
        if (!empty($file))
        {
          ...
          if (strpos($file, "/$dst_disk/") !== false)
          {
            $dst = substr($file, strpos($file, "/$dst_disk/") + strlen("/$dst_disk/"));
