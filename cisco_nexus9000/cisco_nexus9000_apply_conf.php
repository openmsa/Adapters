...
function cisco_nexus9000_apply_conf($configuration, $push_to_startup = false)
{
  ...
  if ($protocol === 'SSH' && $push_to_startup === false)
  {
    ...
    try
    {
      ...
      if ($ret === SMS_OK)
      {
        ...
        if ($push_to_startup)
        {
          $SMS_OUTPUT_BUF = copy_to_running("copy $file_name startup-config");
          save_result_file($SMS_OUTPUT_BUF, "conf.error");
        }
        else
        {
          $SMS_OUTPUT_BUF = copy_to_running("copy $file_name running-config");
          save_result_file($SMS_OUTPUT_BUF, "conf.error");
        }
        ...
        sendexpectone(__FILE__ . ':' . __LINE__, $sms_sd_ctx, "delete $file_name", "[y]");
  ...
  if ($is_ztd || $push_to_startup)
  {
    ...
    sendexpectone(__FILE__ . ':' . __LINE__, $sms_sd_ctx, "management");
...
# skip to check transfer completion - I gave up
#  if (!strpos($SMS_OUTPUT_BUF, 'Copy complete'))
  if (false)
  {
