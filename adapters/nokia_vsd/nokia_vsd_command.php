...
class nokia_vsd_command extends generic_command
{
   ...
   * The operation section: It has to have all operations that the Object is going to use separated by '##', the fields should start with the
   *                       HTTP method name followed by a '#' and then the operation (Example: POST#nova)
   * The XPath section: This section has to have the same number of XPaths separated by '##' as many as operations
   * The config section: This section can have one or more (upto as many as operations/XPaths), each part of config is separated by '##'.
  ...
  function eval_CREATE()
  {
    ...
    {
      $operation_str = trim($create->evaluate_operation());
  ...
  function eval_UPDATE()
  {
    ...
    {
      $operation_str = trim($update->evaluate_operation());
      $operations = explode("##", $operation_str);
