...
function f5_rest_connect($sd_ip_addr = null, $login = null, $passwd = null, $port_to_use = null) {
	global $sms_sd_ctx;
	global $model_data;
	...
	if (isset($sd->SD_CONFIGVAR_list['AUTH_MODE'])) {
        ...
        echo  "f5_rest_connect: setting authentication mode to: {$auth_mode}\n";

	if (isset($sd->SD_CONFIGVAR_list['MANAGEMENT_PORT'])) {
                $port_to_use = trim($sd->SD_CONFIGVAR_list['MANAGEMENT_PORT']->VAR_VALUE);
                echo "f5_rest_connect: using management port: " . $port_to_use . "\n";
	}

	echo "f5_rest_connect: using connection class: " . $class . "\n";
	$sms_sd_ctx = new $class ( $sd_ip_addr, $login, $passwd, "", $port_to_use );

         $sms_sd_ctx->auth_mode = 'BASIC';
	...
	if ($sms_sd_ctx->auth_mode == "token" || $sms_sd_ctx->auth_mode == "auth-key" || $sms_sd_ctx->auth_mode == "jns_api_v2") {
	    ...
	    if (isset($sd->SD_CONFIGVAR_list['AUTH_KEY'])) {
   	    ...
   	    echo  "f5_rest_connect: setting AUTH_KEY to: {$sms_sd_ctx->key}\n";
		...
		echo  "f5_rest_connect: setting SIGNIN_REQ_PATH to: {$sms_sd_ctx->sign_in_req_path}\n";
		...
		echo  "f5_rest_connect: setting authentication header to: {$sms_sd_ctx->auth_header}\n";
	}
	...
	$sms_sd_ctx->http_header_list = explode("|", $http_header_str);
	echo "f5_rest_connect: setting HTTP header to: ".print_r($sms_sd_ctx->http_header_list, true)."\n";
	...
	echo  "f5_rest_connect: setting HTTP protocol to: {$sms_sd_ctx->protocol}\n";
	...
	echo  "f5_rest_connect: setting HTTP timeout to: {$sms_sd_ctx->conn_timeout}\n";
