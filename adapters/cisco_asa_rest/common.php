...
function scp_from_router($src, $dst)
{
  ...
  cisco_asa_rest_disconnect(true);
...
function scp_to_router($src, $dst)
{
  ...
  $ret_scp = exec_local(__FILE__ . ':' . __LINE__, "/opt/sms/bin/sms_scp_transfer -s $src -d $dst_disk:/$dst -l $login -a $sd_ip_addr -p $passwdi -P $sd_mgt_port", $output);
  ...
  // Check file size
  check_file_size($src, $dst, $dst_disk);
