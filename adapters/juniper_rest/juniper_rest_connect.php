...
class DeviceConnection extends GenericConnection {
	...
	protected function execute_curl_command($origin, $rest_cmd, $curl_cmd) {
		...
		/*
		The loop is proposed to make output correct (<tag>value</tag>) in case if original output contains new line like
		<tag>
		value
		</tag>
		*/
		foreach ($output_array as $key => &$value) {
			if ($key > 2) {
				$nminus2 = $output_array[$key-2];
				$nminus1 = $output_array[$key-1]; 
				if ((preg_match('@^<[^\/<>]+?>@', $nminus2) === 1) and (preg_match('@^[^<].+?$@', $nminus1) === 1) and (preg_match('@^<\/[^\/<>]+?>@', $value) === 1)) {
					unset($output_array[$key-1]);
					unset($output_array[$key-2]);
					...
					if (strpos($curl_cmd, 'application/xml') !== false) {
						$line = trim($line);
                                                if (strpos($line, '<') === 0) {
                                                        ...
                                                        if ((preg_match('/<[^<>]+?:([^<>]+?)>/', $line) === 1) and (strpos($a, 'xml version=') === false)) {
                                                        	$line = preg_replace('/<[^<>]+?:([^<>]+?)>/', '<\1>', $line);
                                                        }
                                                } else {
                                                        $line = '';      
		...
		if (strpos($curl_cmd, "Content-Type: application/json")) {
			...
			if ((strpos($curl_cmd, '<load-configuration>') !== false) and (strpos($result, '<load-success/>') === False)) {
				throw new SmsException ( "Load configuration is failed", ERR_SD_CMDFAILED );
			}
			if ((strpos($curl_cmd, '<commit>') !== false) and (strpos($result, '<commit-success/>') === False)) {
				throw new SmsException ( "Commit is failed", ERR_SD_CMDFAILED );
			}
			if (strpos($curl_cmd, 'POST') !== False) {
				$result = "<root>\n".$result."</root>";
			}
...
function juniper_rest_connect($sd_ip_addr = null, $login = null, $passwd = null, $port_to_use = null) {
	global $sms_sd_ctx;
	global $model_data;
	...
	if (isset($sd->SD_CONFIGVAR_list['AUTH_MODE'])) {
        ...
        echo  "juniper_rest_connect: setting authentication mode to: {$auth_mode}\n";

	if (isset($sd->SD_CONFIGVAR_list['MANAGEMENT_PORT'])) {
                $port_to_use = trim($sd->SD_CONFIGVAR_list['MANAGEMENT_PORT']->VAR_VALUE);
                echo "juniper_rest_connect: using management port: " . $port_to_use . "\n";
	}

	echo "juniper_rest_connect: using connection class: " . $class . "\n";
	...
	if ($sms_sd_ctx->auth_mode == "token" || $sms_sd_ctx->auth_mode == "auth-key" || $sms_sd_ctx->auth_mode == "jns_api_v2") {
	    ...
	    if (isset($sd->SD_CONFIGVAR_list['AUTH_KEY'])) {
   	    ...
   	    echo  "juniper_rest_connect: setting AUTH_KEY to: {$sms_sd_ctx->key}\n";
		...
		echo  "juniper_rest_connect: setting SIGNIN_REQ_PATH to: {$sms_sd_ctx->sign_in_req_path}\n";
		...
		echo  "juniper_rest_connect: setting authentication header to: {$sms_sd_ctx->auth_header}\n";
	}
	...
	$sms_sd_ctx->http_header_list = explode("|", $http_header_str);
	echo "juniper_rest_connect: setting HTTP header to: ".print_r($sms_sd_ctx->http_header_list, true)."\n";
	...
	echo  "juniper_rest_connect: setting HTTP protocol to: {$sms_sd_ctx->protocol}\n";
	...
	echo  "juniper_rest_connect: setting HTTP timeout to: {$sms_sd_ctx->conn_timeout}\n";
...
// ------------------------------------------------------------------------------------------------
// Disconnect
function juniper_rest_disconnect() {
	global $sms_sd_ctx;
	$sms_sd_ctx = null;
	return SMS_OK;
}
