<?php
/*
 * Version: $Id$
 * Created: May 13, 2011
 * Available global variables
 *  $sms_csp            pointer to csp context to send response to user
 *  $sms_sd_ctx         pointer to sd_ctx context to retreive usefull field(s)
 *  $sms_sd_info        pointer to sd_info structure
 *  $SMS_RETURN_BUF     string buffer containing the result
 */

require_once 'smsd/sms_common.php';
require_once load_once('huawei_generic', 'device_apply_conf_errors.php');
require_once "$db_objects";

function device_apply_conf($configuration, $push_to_startup = false)
{
  global $sdid;
  global $sms_sd_ctx;
  global $sendexpect_result;
  global $apply_errors;

  $network = get_network_profile();
  $SD = &$network->SD;

  $ret = save_result_file($configuration, "conf.applied");
  if ($ret !== SMS_OK)
  {
    return $ret;
  }

  $config_prompt = '['.$sms_sd_ctx->getPrompt();
  $login_prompt = '<'.$sms_sd_ctx->getPrompt().'>';

  // Switch to config mode
  sendexpectone(__FILE__ . ':' . __LINE__, $sms_sd_ctx, 'system-view', $config_prompt);

  $SMS_OUTPUT_BUF = '';
  $ERROR_BUFFER = '';
  unset($tab);
  $tab[0] = $config_prompt;
  $tab[1] = $login_prompt;

  $buffer = $configuration;
  $line = get_one_line($buffer);
  while ($line !== false)
  {
    $line = trim($line);
    if (strpos($line, '#') !== 0 && !empty($line))
    {
      if (preg_match('@sysname\s+(?<prompt>.*)@', $line, $matches) === 1)
      {
        $sms_sd_ctx->setPrompt($matches['prompt']);
        $config_prompt = '['.$sms_sd_ctx->getPrompt();
        $login_prompt = '<'.$sms_sd_ctx->getPrompt().'>';
        $tab[0] = $config_prompt; //prompt changes on hostname update
        $tab[1] = $login_prompt;
      }

      $index = sendexpect(__FILE__ . ':' . __LINE__, $sms_sd_ctx, $line, $tab);
      $SMS_OUTPUT_BUF .= $sendexpect_result;

      foreach ($apply_errors as $apply_error)
      {
        if (preg_match($apply_error, $sendexpect_result, $matches) > 0)
        {
          $ERROR_BUFFER .= "!";
          $ERROR_BUFFER .= "\n";
          $ERROR_BUFFER .= $line;
          $ERROR_BUFFER .= "\n";
          $ERROR_BUFFER .= $apply_error;
          $ERROR_BUFFER .= "\n";
          $ERROR_BUFFER .= $sendexpect_result;
          $ERROR_BUFFER .= "\n";
          $SMS_OUTPUT_BUF = '';
        }
      }
    }
    $line = get_one_line($buffer);
  }
  sendexpectone(__FILE__ . ':' . __LINE__, $sms_sd_ctx, 'return', $login_prompt);
  sendexpectone(__FILE__ . ':' . __LINE__, $sms_sd_ctx, 'save', '(y/n)');
  sendexpectone(__FILE__ . ':' . __LINE__, $sms_sd_ctx, 'y', $login_prompt);

  if (!empty($ERROR_BUFFER))
  {
    save_result_file($ERROR_BUFFER, "conf.error");
    $SMS_OUTPUT_BUF = $ERROR_BUFFER;
    sms_log_error(__FILE__ . ':' . __LINE__ . ": [[!!! $SMS_OUTPUT_BUF !!!]]\n");
    return ERR_SD_CMDFAILED;
  }
  else
  {
    save_result_file("No error found during the application of the configuration", "conf.error");
  }


  return SMS_OK;
}

?>
