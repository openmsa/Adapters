...
require_once load_once('nokia_cloudband', 'nokia_cloudband_apply_conf.php');
...
{
  $ret = nokia_cloudband_connect($login, $passwd);
  return $ret;
}
...
{
  $ret = nokia_cloudband_disconnect();
  return $ret;
}
...
function sd_apply_conf($configuration, $need_sd_connection)
{
  ...
  $ret = nokia_cloudband_apply_conf($configuration);
